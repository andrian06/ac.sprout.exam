...
export class EmployeeCalculate extends Component {
    ...
    handleSubmit(e) {
        ...
        const { typeId, absentDays, workedDays } = this.state;

        // Validate absentDays and workedDays as required
        if (typeId === 1 && (absentDays === '' || absentDays < 0 || absentDays > 22)) {
            alert('Absent Days must be between 0 and 22.');
            return;
        } else if (typeId === 2 && (workedDays === '' || workedDays < 0 || workedDays > 22)) {
            alert('Worked Days must be between 0 and 22.');
            return;
        }
        ...
        let contents = this.state.loading
            ...
            : <div>
                <form>
                    ...
                    <div className="form-row">
                        <div className='form-group col-md-12'>
                            <label>Net Income: <b>{this.state.netIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</b></label>
                        </div>
                    </div>
